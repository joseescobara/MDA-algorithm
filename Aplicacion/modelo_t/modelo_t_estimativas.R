...
intercept.armcircunferences <- coef.t[1,1] 
age.armcircunferece <- coef.t[2,1]
gender.armcircunference <- coef.t[3,1]
tleche.armcircunference <- coef.t[4,1]

#wigth coefficients

intercept.weight <- coef.t[1,2] 
age.weight <- coef.t[2,2]
gender.weight <- coef.t[3,2]
tleche.weight <- coef.t[4,2]

#length coefficients

intercept.length <- coef.t[1,3]
age.length <- coef.t[2,3]
gender.length <- coef.t[3,3]
tleche.length <- coef.t[4,3]
...
table4 <- table_estim
capture.output(print(table4, print.gap=3), file = "C:/Users/Jose/OneDrive - Universidad de Antioquia/Escritorio/Maestria/Tesis/Algoritmo MDA/Aplicacion_Quantile/Datos Reales/modelo_t_aplication/Table_4.txt")
