...
muestra_slash_completa_50 <- function(m_dis, B_1, M_1, nu_1, N_muestras = 1000){
  lista_muestras <- list()
  set.seed(12365)
  ...
  for (i in 1:N_muestras){
    ...
    d_comp <- sum(dista_maha_indiv(muestra_ten, m_dis, B_1, M_1))
    if(d_comp >=  319 & d_comp <= 394){
...
muestra_faltantes_slash_50 <- function(muestras, m_diseño, B_1, M_1){
  ...
  if(length(muestras_1_analizar) !=0 ){
    ...
    for (i in 1:corredor){
      ...
      if(prop_faltantes[1] >= prop_faltantes[2] & prop_faltantes[2] >= 
         prop_faltantes[3] & d_comp >= 224 & d_comp <= 394){
