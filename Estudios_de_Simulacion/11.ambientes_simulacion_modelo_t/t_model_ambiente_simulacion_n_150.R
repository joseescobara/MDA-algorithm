...
muestra_t_completa_150 <- function(m_diseno, B_1, M_1, nu_1, N_muestras = 1000){
  lista_muestras <- list()
  conta <- 1
  set.seed(74365)
  semillas <- sample(1:3312513, N_muestras)
  for (i in 1:N_muestras) {
    set.seed(semillas[i])
    muestra_ten <- m_diseno%*%B_1 + rmvt(nrow(m_diseno), sigma = M_1, 
                                         df = nu_1)
    d_comp <- sum(dista_maha_indiv(muestra_ten, m_diseno, B_1, M_1)) #asegurar 
    #atípicos
    if(d_comp >= 833 & d_comp <= 883){
...
detecta_atipicos_0 <- function(muestras, m_diseño, B_1, M_1){
  ...
  for (i in 1:length(muestras)){
    ...
    #cercanos a su media. 
    menores_1 <- which(d_m > 1.5 & d_m < 15) #aquí valido cuáles son esos individuos
...
muestra_faltantes_t_150 <- function(muestras, m_diseño, B_1, M_1){
  ...
  if(length(muestras_1_analizar) !=0 ){
    ...
    for (i in 1:corredor){
      ...
      if(prop_faltantes[1] >= prop_faltantes[2] & prop_faltantes[2] >= 
         prop_faltantes[3] & d_comp >=  540 & d_comp <= 883){
