...
muestra_t_completa <- function(m_dis, B_1, M_1, nu_1, N_muestras = 1000){
  lista_muestras <- list()
  set.seed(12365)
  semillas <- sample(1:132513, N_muestras)
  for (i in 1:N_muestras) {
    set.seed(semillas[i])
    lista_muestras[[i]] <- m_dis%*%B_1 + rmvt(nrow(m_dis), sigma = M_1, df = nu_1)
  }
...
muestra_faltantes_t <- function(muestras, m_diseño, B_1, M_1){
