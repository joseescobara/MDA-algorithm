...
muestra_t_completa_50 <- function(m_dis, B_1, M_1, nu_1, N_muestras = 1000){
  lista_muestras <- list()
  set.seed(12365)
  semillas <- sample(1:1312513, N_muestras)
  conta <- 1
  for (i in 1:N_muestras) {
    set.seed(semillas[i])
    muestra_ten <- m_dis%*%B_1 + rmvt(nrow(m_dis), 
                              sigma = M_1, df = nu_1)
    d_comp <- sum(dista_maha_indiv(muestra_ten, m_dis, B_1, M_1)) #asegurar 
    if(d_comp >=  261 & d_comp <= 336){
...
detecta_atipicos_0 <- function(muestras, m_diseño, B_1, M_1){
  ...
  for (i in 1:length(muestras)){
    ...
    #cercanos a su media. 
    menores_1 <- which(d_m > 1 & d_m < 15) #aquí valido cuáles son esos individuos
...
muestra_faltantes_t_50 <- function(muestras, m_diseño, B_1, M_1){
  ...
  if(length(muestras_1_analizar) !=0 ){
    ...
    for (i in 1:corredor){
      ...
      muestra_filas2 <- sample(filas2, ceiling(n*(6/173)), prob = p1, replace = F)
      ...
      muestra_filas3 <- sample(filas3, n*(1/173), prob = p2, replace = F)
      ...
      muestra_filas4 <- sample(filas4, n*(1/173), prob = p3, replace = F)
      ...
      if(prop_faltantes[1] >= prop_faltantes[2] & prop_faltantes[2] >= 
         prop_faltantes[3] & d_comp >= 176 & d_comp <= 336){
